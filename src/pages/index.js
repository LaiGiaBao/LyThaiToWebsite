          ...
          <tbody>
                {tintuc.edges.filter(edge => (
                  ...
                  <tr>
                    <td><Link to='/'
                    key={edge.node.id}
                    ><GatsbyImage image={getImage(edge.node.featuredImage)}/></Link></td>
                    <td><Link to='/'>{edge.node.title}</Link><br/><p className="text-secondary fs-6">{edge.node.releaseDate}</p></td>
                  </tr>)
                )}
          </tbody>
        </table>
